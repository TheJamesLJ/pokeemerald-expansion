...
static bool8 MonListHasSpecies(const struct WildPokemonHeader *header, u16 species, enum WildArea area)
{
    ...
    if (info != NULL)
    {
        ...
        {
            u16 currSpecies;
            currSpecies = info->wildPokemon[i].species;
            #if RANDOMIZER_AVAILABLE == TRUE
                if (!IsRandomizationPossible(currSpecies, species))
                    continue;
                currSpecies = RandomizeWildEncounter(currSpecies, header->mapNum, header->mapGroup, area, i);
            #endif
