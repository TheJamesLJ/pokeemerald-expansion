    ...
    else
    {
        *isWaterMon = TRUE;
        ...
        if (*isWaterMon)
            ...
            area = WILD_AREA_LAND;
        species = RandomizeWildEncounter(species, gWildMonHeaders[headerId].mapNum, gWildMonHeaders[headerId].mapGroup, area, index);
    #endif
...
u16 GetLocalWaterMon(void)
{
    ...
    if (headerId != HEADER_NONE)
    {
        ...
        if (waterMonsInfo)
        {
            ...
            species = waterMonsInfo->wildPokemon[index].species;
            #if RANDOMIZER_AVAILABLE == TRUE
                species = RandomizeWildEncounter(species, gWildMonHeaders[headerId].mapNum, gWildMonHeaders[headerId].mapGroup, WILD_AREA_WATER, index);
            #endif
